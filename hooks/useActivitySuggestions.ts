...
const suggestionDatabase = {
  lazy: {
    sunny: [
      {
        title: "Read a book in the garden",
        category: "relax",
        mood: "calm",
        time: "14:00",
      },
    ...
    cloudy: [
      {
        title: "Binge-watch documentaries",
        category: "fun",
        mood: "calm",
        time: "15:00",
      },
      {
        title: "Cook comfort food",
        category: "food",
        mood: "happy",
        time: "18:00",
      },
  ...
  social: {
    sunny: [
      ...
      {
        title: "Park picnic with friends",
        category: "social",
        mood: "happy",
        time: "12:30",
      },
      {
        title: "Outdoor café meetup",
        category: "social",
        mood: "happy",
        time: "10:00",
      },
...
export function useActivitySuggestions() {
  ...
  const getDummySuggestions = (
    ...
    const suggestions =
      suggestionDatabase[mood][
        weatherCondition as keyof (typeof suggestionDatabase)[typeof mood]
      ] || suggestionDatabase[mood].sunny;

    return suggestions.slice(0, 4).map((suggestion, index) => ({
      ...suggestion,
      id: `dummy-suggestion-${Date.now()}-${index}`,
  ...
  const getSuggestions = async (
    ...
    if (useAI) {
      const aiSuggestions = await getAISuggestions(mood, weather, location);
    ...
    console.log("Using dummy suggestions");
    const dummySuggestions = getDummySuggestions(mood, weather);
